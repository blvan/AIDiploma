...
namespace Unity.MLAgentsExamples
{
    ...
    public class ObjectContact : MonoBehaviour
    {
        ...
        void OnCollisionEnter(Collision col)
        {
            ...
            if (col.transform.CompareTag(k_Head))
            {
                if (agent is WalkerAgent thisAgent && col.transform.root.TryGetComponent(out WalkerAgent enemy))
                {
                    enemy.TakeDamage(0.2f);
                    thisAgent.AddReward(0.3f); // награда за удар по голове
                }
            }           
